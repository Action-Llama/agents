...
**Scheduled trigger:** Search across all repositories in your organization for unplanned issues. Run `gh search issues "org:<org> label:<triggerLabel> -label:ready-for-dev -label:in-progress -label:agent-completed author:<author> state:open" --json number,title,body,labels,repository --limit 10`. If no issues found, respond `[SILENT]` and stop.
