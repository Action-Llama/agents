...
- **Never fix bugs in source code.** If you discover a bug (code behavior doesn't match its documented intent, function name, or comments), do NOT fix it and do NOT write a test that asserts the buggy behavior. Instead, open a GitHub issue **on `$REPO` only** (the repo you are improving coverage for — never create issues on other repos). The issue **must** have both the `uci-error` and `ready-for-dev` labels — do not omit either one:
  ...
  ```
  After creating the issue, verify both labels were applied by running `gh issue view <number> --repo $REPO --json labels`.
